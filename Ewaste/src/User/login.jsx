...
export default function Login() {
  ...
  const handlePasswordLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('http://localhost:9000/api/user/login', record);
  ...
  const sendOtp = async () => {
    ...
    setLoading(true);
    try {
await axios.post("http://localhost:9000/api/user/request-otp-login", { email: record.email });
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("http://localhost:9000/api/user/verify-otp-login", {
  email: record.email,
  otp,
});
